...
def train_and_evaluate(k_value, text_widget):
    ...
    all_docs = list(document_labels.keys())
    train_docs, test_docs = train_test_split(all_docs, test_size=0.3)

    # Extract vectors for train and test data
    X_train = np.array([doc_tfidf[doc] for doc in train_docs])
    X_test = np.array([doc_tfidf[doc] for doc in test_docs])

    # Extract labels for train data
    y_train = [document_labels[doc] for doc in train_docs]
